...
SILVER2_PATH = "./intermediate/silver_batting.parquet"
OUT = Path("./final")
...
GOLD_PATH = OUT / "gold_batting.parquet"
...
def validate_gold_table(silver2_df):
    ...
    if df.empty:
        raise ValueError("Silver2 batting table is empty. Cannot publish gold.")
    ...
    numeric_cols = ["is_swing", "is_contact", "is_chase", "is_zone_swing", "is_zone_contact", "is_o_contact"]
...
def publish_gold_batting(silver2_validated):
...
if __name__ == "__main__":
    ...
    print("Loaded Silver2 Batting:", silver2.shape)

    silver2_validated = validate_gold_table(silver2)
    print("Validated Silver2 Batting")

    gold_df = publish_gold_batting(silver2_validated)

    # Increment version
    registry = load_registry()
    old_version = registry.get("gold_batting", {}).get("version", 0)
    new_version = old_version + 1

    # Save gold table
    gold_df.to_parquet(GOLD_PATH)
    print(f"Published Gold Batting to {GOLD_PATH} (version {new_version})")

    # Save registry metadata
    save_registry(gold_df, new_version)

    print("Gold Batting pipeline complete.")
